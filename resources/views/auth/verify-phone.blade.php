...
<div class="auth-header">
    <h1>{{ __('messages.verify_phone') }}</h1>
    <p>{{ __('messages.verify_phone_desc', ['phone' => $phone]) }}</p>
</div>
...
<form method="POST" action="{{ route('phone.verification.verify') }}">
    ...
    <button type="submit" class="btn btn-primary">{{ __('messages.verify_phone_btn') }}</button>
</form>
