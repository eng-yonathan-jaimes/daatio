...
<div class="card" style="max-width:560px;">
    <form method="POST" action="{{ route('products.update', $product->id) }}">
        ...
        <div class="form-group">
            <label for="product_weight">{{ __('messages.metal_weight') }}</label>
        ...
        <div class="form-group">
            <label for="product_state">{{ __('messages.product_state') }}</label>
            <select id="product_state" name="product_state" style="width:100%;padding:0.75rem 1rem;border:1px solid var(--color-border);border-radius:8px;font-size:1rem;font-family:var(--font-body);color:var(--color-text);background:var(--color-card);">
                <option value="In Stock" {{ old('product_state', $product->product_state) === 'In Stock' ? 'selected' : '' }}>{{ __('messages.in_stock') }}</option>
                <option value="Out of Stock" {{ old('product_state', $product->product_state) === 'Out of Stock' ? 'selected' : '' }}>{{ __('messages.out_of_stock') }}</option>
            </select>
        </div>
