...
@section('title', __('messages.new_store'))

@section('main')
<div class="page-header">
    <h1>{{ __('messages.new_store') }}</h1>
    ...
    <form method="POST" action="{{ route('stores.store') }}">
        ...
        <div class="form-group">
            ...
            <select id="store_location" name="store_location" style="width:100%;padding:0.75rem 1rem;border:1px solid var(--color-border);border-radius:8px;font-size:1rem;font-family:var(--font-body);color:var(--color-text);background:var(--color-card);">
                <option value="Physical" {{ old('store_location') == 'Physical' ? 'selected' : '' }}>{{ __('messages.physical') }}</option>
                <option value="Online" {{ old('store_location') == 'Online' ? 'selected' : '' }}>{{ __('messages.online') }}</option>
                <option value="Both" {{ old('store_location') == 'Both' ? 'selected' : '' }}>{{ __('messages.both') }}</option>
            </select>
        </div>
        <div style="display:flex;gap:1rem;margin-top:1.5rem;">
            <button type="submit" class="btn btn-primary">{{ __('messages.create_store') }}</button>
